...
(function renderDonut() {
  ...
  donut.dataset.center = LANGS.length ? 'Top\n' + LANGS[0].name : '';
...
// per week); a day's row is derived from its actual weekday since edge weeks
// from the GitHub calendar can be partial. Heatmap level is bucketed by
// quartiles of the max count seen across the year.
const tip = $('#tip');
(function renderCalendar() {
  ...
  for (const week of weeks) {
    ...
    lastMonth = month;
    // Weeks from the GitHub calendar can be partial at either edge of the
    // year, so a day's grid row must come from its actual weekday, not its
    // index within the (possibly short) week array.
...
function render() {
  ...
  tbody.innerHTML = shown.map((r) =>
    `<tr><td><a class="repo-name" href="${escapeHtml(safeUrl(r.url))}" target="_blank" rel="noopener">${escapeHtml(r.name)}</a><div class="repo-desc">${escapeHtml(r.description)}</div></td>
